...
export default function Subscription() {
  ...
  const featuresPro = ["Ficha de pré-avaliação", "Prontuário eletrônico", "Histórico de Evolução do paciente", "Criar contrato", "Biblioteca de exercícios", "Ranking de progresso do paciente", "Agendar pacientes", "App para pacientes"];
  const featuresPremium = ["Ficha de pré-avaliação", "Criar planos de acompanhamento", "Prontuário eletrônico", "Histórico de Evolução do paciente", "Criar contrato", "Biblioteca de exercícios", "Ranking de progresso do paciente", "Agendar pacientes", "App para pacientes"];
  return <div className="container mx-auto py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className={`border-2 hover:shadow-lg transition-all ${selectedPlan === "pro" ? "border-movebetter-primary" : "border-gray-200"}`}>
          <CardHeader>
            ...
            <CardDescription className="text-lg">
              <span className="text-3xl font-bold">R$ 79</span>/mês
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="space-y-2">
              {featuresPro.map((feature, index) => <li key={index} className="flex items-center gap-2">
          ...
          <CardFooter>
            <Button className={`w-full ${selectedPlan === "pro" ? "bg-movebetter-primary" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`} onClick={() => handleSelectPlan("pro")}>
        ...
        <Card className={`border-2 hover:shadow-lg transition-all ${selectedPlan === "premium" ? "border-movebetter-primary" : "border-gray-200"}`}>
          ...
          <CardContent>
            <ul className="space-y-2">
              {featuresPremium.map((feature, index) => <li key={index} className="flex items-center gap-2">
                  <Check className="h-5 w-5 text-green-500" />
          ...
          <CardFooter>
            <Button className={`w-full ${selectedPlan === "premium" ? "bg-movebetter-primary" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`} onClick={() => handleSelectPlan("premium")}>
      ...
      <div className="mt-8 flex justify-center">
        <Button className="bg-movebetter-primary hover:bg-movebetter-primary/90 px-8 py-6 text-lg" disabled={isLoading || !selectedPlan} onClick={handleSubscribe}>
