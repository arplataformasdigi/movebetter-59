          ...
          <div className="ml-auto flex items-center space-x-4">
            <div className="hidden md:flex items-center space-x-4">
              <Avatar className="h-8 w-8">
                <AvatarImage src="" alt={patientUser?.patients?.name || ""} />
                <AvatarFallback className="bg-movebetter-primary text-white">
                  {patientUser?.patients?.name ? patientUser.patients.name.charAt(0) : "P"}
                </AvatarFallback>
              </Avatar>
              <div>
                <div className="text-sm font-medium">{patientUser?.patients?.name || "Paciente"}</div>
                <div className="text-xs text-muted-foreground">{patientUser?.email}</div>
              </div>
            </div>
          ...
          <div className="mt-auto p-4 border-t border-gray-200">
            <div className="bg-movebetter-light rounded-lg p-4">
              ...
              <p className="text-xs text-gray-600 mt-2">
                45% do seu plano de tratamento completo
              </p>
            </div>
          </div>
        </div>
